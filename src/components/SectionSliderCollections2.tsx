...
import { changeHotCollections, selectHotCollections } from "app/reducers/collection.reducers";
...
const SectionSliderCollections2: FC<SectionSliderCollections2Props> = ({
  ...
  const getHotCollections = (time:any, limit:any) =>  {
    ...
    axios.post(`${config.API_URL}api/collection/get_hot_collections`, { limit: limit, time: time }, {
      headers:
      {
      ...
      >
        {[
          ...
          <NavItem2
            ...
            onClick={() =>{ setTabActive(item.name); setTime(index); }}
          >
