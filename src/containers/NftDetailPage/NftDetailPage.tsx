...
const NftDetailPage: FC<NftDetailPageProps> = ({
  ...
  const { listNFT, cancelSaleNFT, sendToken, acceptSaleNFT }: any = useSigningClient();
  ...
  useEffect(() => {
    socket.on("UpdateStatus", data => {
      if (tokenId) {
        ...
        if (data.data.itemId === tokenId) getNftDetail(tokenId || "");
      }
    });
    (async () => {
  ...
  const onBid = async (bidPrice: number) => {
    ...
    try {
      let txHash = await sendToken(currentUsr.address, bidPrice, globalDetailNFT.tokenId, globalDetailNFT.collection_id.address);
      if (txHash == -1) {
        toast.error("Blockchian network error.");
      }
      else {
        axios.post(`${config.API_URL}api/item/placeAbid`,
          {
  ...
  const cofirmBuy = async () => {
    ...
    // else toast.error((result as any).message);   
    try {
      let txHash = await sendToken(currentUsr.address, globalDetailNFT.price, globalDetailNFT.tokenId, globalDetailNFT.collection_id.address);
      if (txHash == -1) {
        toast.error("Blockchian network error.");
      }
      else {
        axios.post(`${config.API_URL}api/item/buynow`,
          {
            itemId: globalDetailNFT._id,
  ...
  const onPutSale = async (price: number, instant: boolean, period: number) => {
    ...
    // if((result as any).success === true) toast.success((result as any).message);
    // else toast.error((result as any).message);  
    console.log("onPutSale 0 globalDetailNFT ==> ", globalDetailNFT);
    ...
    try {
      let txhash = await listNFT(
        ...
        !instant ? { "Time": [Math.floor(Date.now() / 1000), Math.floor(Date.now() / 1000) + Math.floor(aucperiod)] } : "Fixed",
        price,
        price,
      ...
      if (txhash != -1) {
        //update db
        await axios.post(`${config.API_URL}api/item/updateForSale`, {
          itemId: globalDetailNFT._id,
          period: aucperiod,
          price: price
        }).then(response => {
          if (response.data.code == 0) toast.success("Succeed put on sale.");
  ...
  const removeSale = async () => {
    ...
    // else toast.error((result as any).message);    
    try {
      let txHash = await cancelSaleNFT(currentUsr.address, globalDetailNFT.collection_id.address, globalDetailNFT.tokenId);
      if (txHash == -1) {
        toast.error("Blockchian network error.");
        return;
      }
      else {
        axios.post(`${config.API_URL}api/item/removeFromSale`, {
          itemId: globalDetailNFT._id
        }).then(response => {
          if (response.data.code == 0) toast.success("Succeed in unlisting an item.");
          else toast.error("Server side error");
          getNftDetail(tokenId || "");
        }).catch(error => {
          toast.error("Server side error");
  ...
  const onAccept = async () => {
    ...
    // else toast.error((result as any).message);    
    try {
      let txHash = await acceptSaleNFT(currentUsr.address, globalDetailNFT.collection_id.address, globalDetailNFT.tokenId);
      if (txHash == -1) {
        toast.error("Network connection error.")
      }
      else {
        axios.post(`${config.API_URL}api/item/acceptBid`,
          {
            itemId: globalDetailNFT._id
          }).then(response => {
            if (response.data.code == 0) {
  ...
  const calculateTimeLeft = (created: number, period: number, time: number) => {

    // var createdTime = (new Date(created * 1000)).getTime();
    let difference = created * 1000 + period * 1000 - time;
  ...
  useEffect(() => {
    ...
    const intVal = setInterval(() => calculateTimeLeft(globalDetailNFT?.auctionStarted, globalDetailNFT?.auctionPeriod, curTime), 1000)
      ...
      <div className="divide-y divide-neutral-100 dark:divide-neutral-800">
        <div className="space-y-5 pb-9">
          ...
          <div className="flex flex-col space-y-4 text-sm sm:flex-row sm:items-center sm:space-y-0 sm:space-x-8">
            <div className="flex items-center cursor-pointer " onClick={() => navigate(`/page-author/${globalDetailNFT?.creator?._id || ""}/${(globalDetailNFT as any).owner?.view_mode || 0}`)}>
            ...
            <div className="space-y-5">
              ...
              <div className="flex space-x-5 sm:space-x-10">
                ...
                <div className="flex flex-col ">
                  ...
                  <span className="sm:text-lg text-neutral-500 dark:text-neutral-400">seconds</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid w-full grid-cols-1 gap-10 lg:grid-cols-2 md:gap-14">
            <div className="space-y-8 lg:space-y-10">
              ...
              <AccordionInfo
                ...
                contractAddress={globalDetailNFT?.collection_id?.cw721address || ""}
